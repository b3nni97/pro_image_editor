...
typedef InteractiveViewerScrollPhysicsWidgetBuilder = Widget Function(
...
/// {@end-tool}
@immutable
class InteractiveViewerScrollPhysics extends StatefulWidget {
  ...
  /// large child.
  InteractiveViewerScrollPhysics.builder({
    super.key,
    this.clipBehavior = Clip.hardEdge,
    this.panAxis = PanAxis.free,
    this.boundaryMargin = EdgeInsets.zero,
  ...
  final InteractiveViewerScrollPhysicsWidgetBuilder? builder;
  ...
  static Quad getAxisAlignedBoundingBox(Quad quad) {
    final double minX = math.min(quad.point0.x,
        math.min(quad.point1.x, math.min(quad.point2.x, quad.point3.x)));
    final double minY = math.min(quad.point0.y,
        math.min(quad.point1.y, math.min(quad.point2.y, quad.point3.y)));
    final double maxX = math.max(quad.point0.x,
        math.max(quad.point1.x, math.max(quad.point2.x, quad.point3.x)));
    final double maxY = math.max(quad.point0.y,
        math.max(quad.point1.y, math.max(quad.point2.y, quad.point3.y)));
    return Quad.points(Vector3(minX, minY, 0), Vector3(maxX, minY, 0),
        Vector3(maxX, maxY, 0), Vector3(minX, maxY, 0));
  }
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveViewerScrollPhysics.getNearestPointOnLine(
        point,
        quad.point0,
        quad.point1,
      ),
    ...
    for (final Vector3 closePoint in closestPoints) {
      final double distance = math.sqrt(math.pow(point.x - closePoint.x, 2) +
  ...
  @override
  State<InteractiveViewerScrollPhysics> createState() =>
      _InteractiveViewerScrollPhysicsState();
}

class _InteractiveViewerScrollPhysicsState
    extends State<InteractiveViewerScrollPhysics>
...
// Return a new matrix representing the given matrix after applying the given
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    final Quad boundariesAabbQuad =
        _getAxisAlignedBoundingBoxWithRotation(_boundaryRect, _currentRotation);
    ...
    if (widget.scrollPhysics != null) {
      ...
      final Offset currentOffset =
          _getMatrixTranslation(_transformer.value) * -1;
    ...
    return matrix.clone()
      ..setTranslation(
        Vector3(unidirectionalCorrectedTotalTranslation.dx,
            unidirectionalCorrectedTotalTranslation.dy, 0.0),
      );
  }
  ...
  Matrix4 _matrixScale(Matrix4 matrix, double scale) {
    ...
    if (widget.scrollPhysics != null) {
      ...
      if (_gestureType == _GestureType.scale &&
          ...
          ((desiredScale < widget.minScale) ||
              (desiredScale > widget.maxScale))) {
        final Size contentSize =
            _boundaryRect.isInfinite ? _childSize() : _boundaryRect.size;
        ...
        final double desiredContentHeight =
            contentSize.height * incrementalScale;
    ...
    switch (_gestureType) {
      case _GestureType.pan:
        if (widget.scrollPhysics != null) {
          ...
          final metricsX = _calculateScrollMetrics(
            adjustedOffset.dx,
            AxisDirection.right,
            scale: targetScale,
          );
          final metricsY = _calculateScrollMetrics(
            adjustedOffset.dy,
            AxisDirection.down,
            scale: targetScale,
          );
          ...
          if (details.velocity.pixelsPerSecond.distance <=
                  widget.scrollPhysics!.minFlingVelocity &&
              !metricsX.outOfRange &&
              !metricsY.outOfRange &&
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent) {
      if (event.kind == PointerDeviceKind.trackpad &&
        ...
        _transformer.value = _matrixTranslate(
            _transformer.value, newFocalPointScene - focalPointScene);
    ...
    _transformer.value = _matrixTranslate(
        _transformer.value, focalPointSceneScaled - focalPointScene);
  ...
  void _handleInertiaAnimation() {
    ...
    if (widget.scrollPhysics != null) {
      ...
      // Translate such that the resulting translation is _animation.value.
      final Offset animationScene = _transformer.toScene(_animation!.value);
  ...
  ScrollMetrics _calculateScrollMetrics(
      double pixels, AxisDirection axisDirection,
      {double? scale}) {
    final panBoundaries = _computePanBoundaries(
      viewportSize: _viewport.size,
      scale: scale ?? _transformer.value.getMaxScaleOnAxis(),
    ...
    final scrollMetrics = FixedScrollMetrics(
      ...
      viewportDimension:
          axis == Axis.horizontal ? _viewport.width : _viewport.height,
  ...
  bool _shouldAllowScale(double proposedScale) {
    if (widget.scrollPhysics == null) {
      return proposedScale >= widget.minScale &&
          proposedScale <= widget.maxScale;
    }
    ...
    final double adjustmentX = widget.scrollPhysics!
        .applyBoundaryConditions(metricsX, desiredContentWidth);
    final double adjustmentY = widget.scrollPhysics!
  ...
  void _handleScaleAnimation() {
    ...
    final Offset focalPointSceneScaled =
        _transformer.toScene(_scaleAnimationFocalPoint);
    _transformer.value = _matrixTranslate(
        _transformer.value, focalPointSceneScaled - referenceFocalPoint);
  }
  ...
  @override
  void didUpdateWidget(InteractiveViewerScrollPhysics oldWidget) {
...
double _getFinalTime(double velocity, double drag,
    {double effectivelyMotionless = 0.5}) {
...
Offset _round(Offset offset) {
  return Offset(double.parse(offset.dx.toStringAsFixed(9)),
...
class CombinedSimulation extends Simulation {
  CombinedSimulation({
    required this.simulationX,
    required this.simulationY,
    required this.simulationScale,
  });

  final Simulation simulationX;
  final Simulation simulationY;
  final Simulation simulationScale;
  ...
  double dx(double time) {
    ...
    final double dxScale = simulationScale.dx(time);
    return math.sqrt(dxX * dxX + dxY * dxY + dxScale * dxScale);
  }

  @override
  bool isDone(double time) {
    return simulationX.isDone(time) &&
        simulationY.isDone(time) &&
        simulationScale.isDone(time);
  }
}
